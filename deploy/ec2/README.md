...
2. SSH + `rsync` syncs repo content to `/opt/smart-outfit` on EC2.
3. Runs `deploy/ec2/deploy-ec2.sh` remotely.
4. Verifies `http://<EC2_HOST>/api/health`.

Add these GitHub repo secrets:

- `EC2_HOST` (Elastic IP or domain)
- `EC2_USER` (`ubuntu`)
- `EC2_SSH_KEY` (contents of private key)
- `EC2_SSH_PORT` (optional, defaults to `22`)

Important:

- Keep `/opt/smart-outfit/.env` on EC2. The workflow intentionally does not sync `.env`.
